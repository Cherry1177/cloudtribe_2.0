...
const DriverAvailableTimes: React.FC<{ driverId: number }> = ({ driverId }) => {
  ...
  const fetchTimeSlots = async () => {
    try {
      console.log('Fetching time slots for driver:', driverId);
      const response = await fetch(`/api/drivers/${driverId}/times`);
      const data = await response.json();
      console.log('Raw time slots data:', data);
      if (Array.isArray(data)) {
          setTimeSlots(data);
          console.log('Set timeSlots:', data);
        } else {
        ...
        >
          ...
          <div className="mt-4">
            ...
            <div className="mt-6">
              ...
              {(() => {
                console.log('Rendering timeSlots:', timeSlots, 'length:', timeSlots.length);
                return null;
              })()}
              {timeSlots.length > 0 ? (
                <div className="space-y-4">
                  ...
                  {(() => {
                    const activeSlots = timeSlots.filter(slot => {
                      if (!slot.start_time || !slot.date) return false;
                      const now = new Date();
                      const [hours, minutes] = slot.start_time.split(':');
                      const slotDateTime = new Date(slot.date);
                      slotDateTime.setHours(parseInt(hours), parseInt(minutes), 0, 0);
                      console.log('Checking slot:', slot, 'now:', now, 'slotDateTime:', slotDateTime, 'isActive:', slotDateTime >= now);
                    ...
                    <div>
                      <h4 className="text-md font-semibold text-green-600 mb-2">✅ 可用時間</h4>
                  ...
                  {timeSlots.filter(slot => {
                    ...
                    <div>
                      <h4 className="text-md font-semibold text-red-600 mb-2">⏰ 已過期時間</h4>
                      <div className="grid grid-cols-1 gap-4">
                        {timeSlots.filter(slot => {
                          if (!slot.start_time || !slot.date) return false;
                          const now = new Date();
