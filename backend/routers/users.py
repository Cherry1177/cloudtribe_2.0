...
async def verify_phone(user_id: int, data: dict, conn: Connection = Depends(get_db_connection)):
    """
    Verify if provided phone matches user's phone in DB.
    """
    cur = conn.cursor()
    try:
        ...
        db_phone = row[0]
        is_valid = (data.get("phone") == db_phone)
...
async def update_profile(user_id: int, req: UpdateProfileRequest, conn: Connection = Depends(get_db)):
    """
    Update user's name and/or password by user_id.

    Args:
        user_id (int): The ID of the user to update.
        req (UpdateProfileRequest): The updated name and/or password.
        conn (Connection): The database connection.
    ...
    try:
        ...
        conn.commit()
        log_event("USER_PROFILE_UPDATE", {
            "user_id": user_id,
            "updated_fields": [field.split('=')[0].strip() for field in updates]
        })
